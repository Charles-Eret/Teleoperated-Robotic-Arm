...
class Robot:
    ...
    def read_position(self, rad=False, deg=False, tries=2):
        """
        ...
        for id in self.servo_ids:
            ...
            if position > 2 ** 31:
                position -= 2 ** 32
            if deg:
            	position = ((position - 2048) / 2048) * 180
            if rad:
            	position = ((position - 2048) / 2048) * math.pi
    ...
    def print_velocity_limit(self):
        """
        Prints the current velocity limit of each servo.
        """
        for motor_id in self.servo_ids:
            limit = self.dynamixel.read_velocity_limit(motor_id)
            print(f"Motor ID: {motor_id}, Current Velocity Limit: {limit}")
    ...
    def limit_velocity(self, limit):
        """
        Sets the maximum velocity for the servos.
        @param limit: velocity limit (0 ~ max velocity of the servo)
        """
        print(f"Limiting velocity to {limit}")
        for motor_id in self.servo_ids:
    ...
    def apply_haptic_feedback(self, other_robot_positions: list):
        #for haptic feedback based on positional difference
        #for upper and lower torque limits
        hysteresis_upper_threshold: float = 440.0
        hysteresis_lower_threshold: float = -440.0
        #no-torque zone
        dead_zone_size: float = 15*11.38 #11.38 is the scale factor for degrees->servo position
        #scale factor for position-to-torque 
        torque_scale_factor: float = (885/2)*(1/(30*11.38)) #max haptic torque will be at 30 degrees

        robot_positions = self.read_position()
        pwm_values = []
        for motorID in range(len(self.servo_ids)):
            position_diff = other_robot_positions[motorID] - robot_positions[motorID]
            if abs(position_diff) < dead_zone_size: #ignoring torque if in dead zone
                pwm_values.append(0)
                continue
            if(position_diff < 0):
                pwm_value = (position_diff + dead_zone_size)*torque_scale_factor #converting positional difference to a pwm
            else:
                pwm_value = (position_diff - dead_zone_size)*torque_scale_factor
            #keeping in hysteresis bounds:
            pwm_value = max(pwm_value, hysteresis_lower_threshold)
            pwm_value = min(pwm_value, hysteresis_upper_threshold)
            pwm_values.append(int(pwm_value))
        pwm_values[-1] += 200 #applying constant torque to gripper
        self.set_pwm(pwm_values)
        #print(pwm_values)
